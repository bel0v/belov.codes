...
function main() {
  ...
  */
  ...
  textCtx1.fillText('COMING SOON', textCanvas1.width / 2, textCanvas1.height / 2)
  ...
  function drawNoise() {
    ...
    for (let i = 0; i < DOTS_NUM; i += 1) {
      ...
      const radius = 100
      const x = radius * Math.sin(angle) * 2 * perlin(1 * Math.sin(angle) + time / 100, time / 100)
      const y = radius * Math.cos(angle) * 2 * perlin(1 * Math.cos(angle) + time / 100, time / 100)
      maskCtx.lineTo(x, y)
    }
    maskCtx.closePath()
    maskCtx.fill()
